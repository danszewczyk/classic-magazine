...
***************************************************************/
	function pego_javascripts() {
		...
		wp_register_script('classie', get_template_directory_uri() . '/js/classie.js','','',true);
		wp_register_script('dialogFx', get_template_directory_uri() . '/js/dialogFx.js','','',true);
...
Menu declaration
***************************************************************/
	register_nav_menu( 'primary', esc_html__( 'Navigation Menu', 'inkstory' ) );
...
***************************************************************/
	function pego_new_widgets_init() {	
		...
		register_sidebar(array(
			'name' => 'Above footer fullwidth area',
			'id' => 'above-footer-fullwidth-area',
			'before_widget' => '<div id="%1$s" class="widget animationClass %2$s">',
			'after_widget' => '</div>',
			'before_title' => '<h1 class="widget-title-insta">',
			'after_title' => '</h1>',
		));
...
***************************************************************/	
	...
	if (is_admin()){
	  ...
	  //Category title text
	  $my_meta->addText($prefix.'category_title_text',array('name'=> 'Category title text'));
	   //Sidebar select
	  $allSidebarss = pego_get_all_sidebars();
	  $my_meta->addSelect($prefix.'category_sidebar',$allSidebarss,array('name'=> esc_html__('Category sidebar ','tax-meta')));
...
<ul class="socials-wrap">
	<?php if (in_array("facebook", $shareArray)) {  ?>
		<li class="social-item">
			<a href="http://www.facebook.com/share.php?u=<?php echo esc_url($permalink);?>"><i class="fa fa-facebook"></i>Facebook</a>
		</li>
        <?php } ?>
...
function pego_get_prev_next_link() {	
	...
	$prevoius_post = get_previous_post();
	if (!empty( $prevoius_post )) {
		$prevoius_link =  get_permalink( $prevoius_post->ID );
		$image_prev = wp_get_attachment_image_src( get_post_thumbnail_id( $prevoius_post->ID), 'thumbnail' );
		if (get_post_format($prevoius_post->ID ) == 'link') { $prevoius_link =  get_post_meta($prevoius_post->ID  , 'pego_post_link_url' , true); }
	?>
		<a href="<?php echo esc_url(get_permalink( $prevoius_post->ID )); ?>" class="prev-post-wrapper">
			<span class="next-post-details left-details">
				<span class="next-post-details-inside">
				<?php
					echo '<span class="nextprev-post-title">'.esc_html($prevoius_post->post_title).'</span>';
					if ($image_prev[0] != '') {
						echo '<span class="nextprev-post-image"><img src="'.esc_url($image_prev[0]).'" alt="'.esc_attr($next_post->post_title).'" /></span>';
					} 
				?>
				</span>
			</span>
...
***************************************************************/	
	...
	include("functions/widget-aboutme.php");
...
function my_theme_register_required_plugins() {
    ...
    $plugins = array(
        ...
        array(
            ...
            'source'             => get_stylesheet_directory() . '/lib/plugins/option-tree.2.4.6.zip', // The plugin source.
        ...
        array(
            ...
            'source'             => get_stylesheet_directory() . '/lib/plugins/zilla-likes-1.0.zip', // The plugin source.
            'required'           => true, // If false, the plugin is only 'recommended' instead of required.
            'version'            => '', // E.g. 1.0.0. If set, the active plugin must be this version or higher.
            'force_activation'   => true, // If true, plugin is activated upon theme activation and cannot be deactivated until theme switch.
            'force_deactivation' => false, // If true, plugin is deactivated upon theme switch, useful for theme-specific plugins.
        ...
        array(
            ...
            'source'             => get_stylesheet_directory() . '/lib/plugins/contact-form-7.4.0.3.zip', // The plugin source.
            'required'           => true, // If false, the plugin is only 'recommended' instead of required.
            'version'            => '', // E.g. 1.0.0. If set, the active plugin must be this version or higher.
            'force_activation'   => true, // If true, plugin is activated upon theme activation and cannot be deactivated until theme switch.
            'force_deactivation' => false, // If true, plugin is deactivated upon theme switch, useful for theme-specific plugins.
