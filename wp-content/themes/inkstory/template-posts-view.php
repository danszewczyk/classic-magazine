<?php
/*
Template Name: Template Posts View
*/
?>  
<?php get_header(); ?>
<?php
		...
		if ($pego_page_home_template_columns == 2) { $columns_class = 'vc_col-sm-6'; }	
		if ($pego_page_home_template_columns == 3) { $columns_class = 'vc_col-sm-4'; }	
		if ($pego_page_home_template_columns == 4) { $columns_class = 'vc_col-sm-3'; }	
		...
		<div id="container"> <!-- start container -->	
			...
			if ($content) {
			?>
			<div class="vc_row wpb_row vc_inner vc_row-fluid mb30">
			...
			<div class="vc_row wpb_row vc_inner vc_row-fluid">
				<div class="<?php echo esc_attr($main_class); ?> wpb_column vc_column_container">
					<div class="wpb_wrapper">
					<?php
						if ($pego_page_titles_show_filter == 'yes') {
							if ($categories) {
							?>
								<ul id="filters" class="option-set" data-option-key="filter">
									<li class="filter-cat"><a href="#filter" data-option-value="*" class="selected">All</a></li>
									<?php
						 			foreach ($categories as $cat_single) {
					 				?>
					 					<li class="filter-cat"><a href="#filter" data-option-value=".<?php echo esc_attr($cat_single->slug); ?>"><?php echo esc_html($cat_single->name); ?></a></li>
				 					<?php
									}	
									?>
								</ul>
											 			...
											 			<?php
															...
															$idGallery = rand(1, 10000);						  		
         													foreach($attachments as $att_id => $attachment) {
																...
																?>
																<a href="<?php echo esc_url($image[0]); ?>" rel="prettyPhoto[gallery<?php echo esc_attr($idGallery); ?>]" >
																	<?php echo $thumbnail; ?>
																</a>
																</figure>
																<?php
