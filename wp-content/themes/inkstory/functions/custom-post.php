...
function hybrid_create_meta_box() {
	global $theme_name;
	add_meta_box( 'post-meta-boxes', esc_html__('General post options','raysmile'), 'post_meta_boxes', 'post', 'normal', 'default' );
}
...
function hybrid_post_meta_boxes() {
	...
	$meta_boxes = array(	
			'pego_post_alternative_title' => array( 
							...
							'title' => esc_html__('Post alternative title ', 'inkstory'), 
							'description' => esc_html__('Insert alternative title for post. You may use span tags for different color.','inkstory'),
							'type' => 'text' ),
			'pego_post_video_url' => array( 
							'name' => 'pego_post_video_url',
							'title' => esc_html__('Video url:', 'inkstory'), 
							'description' => esc_html__('Used for video post format.','inkstory'),
							'type' => 'text' ),
			'pego_post_link_url' => array( 
							'name' => 'pego_post_link_url',
							'title' => esc_html__('Url for posts with link format', 'inkstory'), 
							'description' => esc_html__('Used for link post format.','inkstory'),
							'type' => 'text' ),
			'pego_post_audio_upload' => array( 
							'name' => 'pego_post_audio_upload', 
							'title' => esc_html__('Post audio upload', 'inkstory'), 
							'description' => esc_html__('Upload mp3 for audio post type.', 'inkstory'), 
							'type' => "upload"),
			'pego_post_soundcloud_url' => array( 
							'name' => 'pego_post_soundcloud_url',
							'title' => esc_html__('Soundcloud shortcode:', 'inkstory'), 
							'description' => esc_html__('Used for audio post format.','inkstory'),
							'type' => 'text' ),
			'pego_post_gallery_type' => array( 
							'name' => 'pego_post_gallery_type', 
							'title' => esc_html__('Select type of gallery', 'inkstory'), 
							'description' => esc_html__('Select gallery type. The setting will be used only for posts with gallery format.', 'inkstory'), 
							'type' => "select", 
							'std' => esc_html__('Slideshow','inkstory'),
							'options' => array(esc_html__('Slideshow','inkstory'),esc_html__('Grid','inkstory')))
);
